...
class Courses(models.Model):
    id = models.AutoField(primary_key=True)
    course_name = models.CharField(max_length=255)
    fee = models.DecimalField(max_digits=10, decimal_places=2)  # Add fee field
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    objects = models.Manager()
...
class Payment(models.Model):
    ...
    class Meta:
        unique_together = ('student', 'course')  # Ensure one payment record per student per course

    def __str__(self):
        return f"{self.student.admin.username} - {self.course.course_name} - {self.amount_paid}"
...
def create_user_profile(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        try:
            if instance.user_type == '1':  # Assuming user_type is a string
                ...
                with transaction.atomic():
                    ...
                    course = Courses.objects.get(id=1)  # Ensure this exists
                    session_year = SessionYearModel.objects.get(id=1)  # Ensure this exists
            ...
            # Handle the case where the Course or SessionYear does not exist
            print(f"Error creating user profile for {instance.username}: {e}")
        except Exception as e:
            # Catch any other exceptions
            print(f"An error occurred while creating user profile: {e}")
